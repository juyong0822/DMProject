    ...
    for key, value in keyword.items() :
        for i in range(value):
            ...
            for j in range(len(reason2_pos)):
                ...
                if word not in stopwords and pos[0] != 'S' and pos[0] != 'J' and pos[0] != 'E':
                    reason2.append(word)
            docs.append(reason2)
            print(reason2)
    return docs
...
docs = generate_docs1()
print(docs)
build_dataset(docs, 1500)

"""

tagged_doclist = []
for i in range(doc_num):
    tagged_doclist.append(TaggedDocument(docs[i], ['상고 도로교통법_' + str(i)]))
...
start = time.time()
doc_vectorizer.build_vocab(tagged_doclist)
for epoch in range(doc_vectorizer.iter):
    doc_vectorizer.train(tagged_doclist, total_examples=doc_vectorizer.corpus_count, epochs=doc_vectorizer.iter)
