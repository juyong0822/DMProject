...
def _filter_header(s):
    ...
    string = asstr(s) + "\n"
    for token in tokenizing.generate_tokens(StringIO(string).readline):
        token_type = token[0]
        token_string = token[1]
        if (last_token_was_number and
                token_type == tokenizing.NAME and
                token_string == "L"):
            continue
        else:
            tokens.append(token)
        last_token_was_number = (token_type == tokenizing.NUMBER)
    # removing newline (see above) as python 2.7.5 workaround
    return tokenizing.untokenize(tokens)[:-1]
